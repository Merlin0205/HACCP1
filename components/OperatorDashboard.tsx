...
import { createOperator, createPremise } from '../services/firestore';
...
interface OperatorDashboardProps {
  ...
  onAddNewOperator: () => void; // Udržujeme pro kompatibilitu, ale už se nepoužívá
  onEditOperator: (operatorId: string) => void;
  onDeleteOperator: (operatorId: string) => void;
  onAddPremise: (operatorId: string) => void; // Udržujeme pro kompatibilitu, ale už se nepoužívá
  ...
  onOperatorAdded?: (operator: Operator) => void; // Callback pro aktualizaci seznamu
  onPremiseAdded?: (premise: Premise) => void; // Callback pro aktualizaci seznamu
}
...
export const OperatorDashboard: React.FC<OperatorDashboardProps> = ({
  ...
  const [premiseOperatorId, setPremiseOperatorId] = useState<string | null>(null);
  ...
  const getPremiseMetrics = (premiseId: string) => {
    ...
    if (lastCompletedAudit) {
      ...
      if (answers.length > 0) {
        const hasNonCompliances = answers.some((answer: any) =>
  ...
  const handleSaveOperatorFromModal = async (operatorData: Omit<Operator, 'id'>) => {
    try {
      ...
      // Zavolat callback pro aktualizaci seznamu (pokud existuje)
      if (onOperatorAdded) {
        onOperatorAdded(newOperator);
      } else {
        // Fallback: reload stránky pokud není callback
        window.location.reload();
      }
    } catch (error: any) {
      console.error('[OperatorDashboard] Error saving operator:', error);
  ...
  const handleSavePremiseFromModal = async (premiseData: Omit<Premise, 'id'>) => {
    try {
      if (!premiseOperatorId) {
        toast.error('Chyba: není vybrán provozovatel');
        return;
      }
      ...
      setPremiseOperatorId(null);
      toast.success('Pracoviště bylo přidáno');
      ...
      console.error('[OperatorDashboard] Error saving premise:', error);
      toast.error('Chyba při přidávání pracoviště: ' + error.message);
    }
  };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto pb-6">
                            ...
                            <div className="flex items-center justify-end gap-2 md:gap-1">
                              <div className="relative group/button">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    setPremiseOperatorId(operator.id);
                                        ...
                                        return (
                                          <div className="flex items-center gap-4 md:gap-2 xl:gap-4">
                                            ...
                                            {metrics.auditResult !== 'unknown' && (
                                              <div className="relative group">
                                                <div className={`flex items-center gap-1.5 md:gap-1 px-2.5 md:px-2 xl:px-2.5 py-1.5 md:py-1 xl:py-1.5 rounded-lg border ${metrics.auditResult === 'compliant'
      ...
      {/* Modal pro přidání nového pracoviště */}
      {premiseOperatorId && (
        <Modal
        ...
        >
          <PremiseForm
            initialData={null}
            operatorId={premiseOperatorId}
            premises={premises.filter(p => p.operatorId === premiseOperatorId)}
