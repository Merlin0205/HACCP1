...
const ImagePreview: React.FC<ImagePreviewProps> = ({ photos, onRemove, onAnalyze }) => {
  ...
  const getImageSrc = (photo: PhotoWithAnalysis): string => {
    // Nejdříve zkontrolujeme, zda je 'file' skutečný souborový objekt.
    // To je klíčové po znovunačtení ze stavu, kde 'file' může být jen prázdný objekt.
    if (photo.file instanceof File) {
      return URL.createObjectURL(photo.file);
    }
    // Pokud to není soubor, spolehneme se na base64 řetězec.
    if (photo.base64) {
      return `data:image/jpeg;base64,${photo.base64}`;
    }
    // Pojistka pro případ, že by chyběl obrázek.
    return '';
  };
