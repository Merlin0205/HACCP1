...
export const InvoiceDetailPage: React.FC<InvoiceDetailPageProps> = ({
  ...
  const loadInvoice = async () => {
    ...
    try {
      ...
      // Pomocná funkce pro konverzi obrázku na base64 pomocí Cloud Function proxy
      // Používáme proxy místo přímého načtení kvůli CORS omezením Firebase Storage
      const convertImageToBase64 = async (url: string): Promise<string | null> => {
        try {
          console.log('[InvoiceDetailPage] Loading image via Cloud Function proxy:', url);
          ...
          console.error('[InvoiceDetailPage] Proxy failed:', error);
          return null;
        }
      };
    ...
    <div className="w-full max-w-5xl mx-auto pb-6 bg-white print:bg-white print:pb-0 print:pt-0 invoice-document-container">
      ...
      <div className="mb-6 print:hidden flex gap-2 no-print">
        ...
        {invoice.status !== 'paid' && invoice.status !== 'cancelled' && (
          <Button variant="primary" onClick={handleMarkAsPaid} disabled={isMarkingAsPaid} className="bg-green-600 hover:bg-green-700 text-white print:hidden">
            {isMarkingAsPaid ? 'Označuji...' : 'Označit jako zaplacenou'}
          </Button>
        )}

        <div className="flex items-center gap-2 ml-4 print:hidden bg-gray-100 p-2 rounded-lg border border-gray-200">
          <ToggleSwitch
            checked={showNewDesign}
            onChange={setShowNewDesign}
            label="Nový design"
            color="teal"
