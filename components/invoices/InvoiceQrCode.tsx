 ...
 */
function sanitizeForQrPayment(input: string): string {
  if (!input) return '';
 ...
 */
function normalizeVariableSymbol(vs: string | undefined | null): string | undefined {
 ...
 */
function formatDateForSpayd(date: Timestamp | string | undefined | null): string | null {
  if (!date) return null;
 ...
 */
function convertToIban(accountNumber: string, bankCode: string): string | null {
...
export const InvoiceQrCode: React.FC<InvoiceQrCodeProps> = ({ invoice }) => {
  ...
  if (invoice.supplier.iban) {
    accValue = invoice.supplier.iban.replace(/\s/g, '').toUpperCase();
  ...
  else if (invoice.supplier.accountNumber && invoice.supplier.bankCode) {
  ...
  else if (invoice.supplier.bankAccount) {
    const bankAccount = invoice.supplier.bankAccount.replace(/\s/g, '').toUpperCase();
    if (bankAccount.startsWith('CZ') && bankAccount.length >= 24) {
      accValue = bankAccount;
    }
  }

  // Pokud nemáme IBAN, QR kód nezobrazíme (ACC je povinný a musí být IBAN)
    ...
    return (
      <div className="w-24 h-24 print:w-20 print:h-20 border-2 border-dashed border-gray-300 rounded flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <span className="text-xs text-gray-400 block mt-1">Chybí IBAN</span>
        </div>
      </div>
    );
  }
  ...
  if (amount.length > 10) {
    // Pokud částka přesahuje limit, nezobrazíme QR kód
    return (
      <div className="w-24 h-24 print:w-20 print:h-20 border-2 border-dashed border-gray-300 rounded flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <span className="text-xs text-gray-400 block">QR Platba</span>
          <span className="text-xs text-gray-400 block mt-1">Částka přesahuje limit</span>
        </div>
      </div>
    );
  }
